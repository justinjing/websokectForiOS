...
static NSString* kInsecureHandshakeURL = @"http://%@/web-pir/socket.io/1/?t=%d%@";
static NSString* kInsecureHandshakePortURL = @"http://%@:%d/socket.io/1/";
static NSString* kSecureHandshakePortURL = @"https://%@:%d/socket.io/1/?t=%d%@";
static NSString* kSecureHandshakeURL = @"https://%@/socket.io/1/?t=%d%@";

//static NSString* kInsecureHandshakeURL = @"http://%@/web-pir/socket.io/";
//static NSString* kInsecureHandshakePortURL = @"http://%@:%d/web-pir/socket.io/";
//static NSString* kSecureHandshakePortURL = @"https://%@:%d/socket.io/1/?t=%d%@";
//static NSString* kSecureHandshakeURL = @"https://%@/socket.io/1/?t=%d%@";
...
//static SocketIO *sh=nil;
//
//
//+(SocketIO *)shareInstance{
//
//    if (sh==nil) {
//        sh=[[self alloc]init];
//    }
...
{   
    ...
    if ([type intValue] != 6 && [type intValue] != 2 && [type intValue] != 0) {
        [encoded addObject:_endpoint];//xxx
    } 
    else {
        [encoded addObject:@""];
    }
    ...
    if (packet.data != nil) {
        ...
        if ([type intValue] == 6) {
            ackpId = [NSString stringWithFormat:@":%@%@", packet.pId, @"+"];//meijin
        }
